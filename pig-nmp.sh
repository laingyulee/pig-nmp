...
show_main_menu() {
    ...
    echo -e "  ${MENU_NUM_COLOR}5)${C_RESET} System Status & Manager"
    echo -e "  ${MENU_NUM_COLOR}6)${C_RESET} Uninstall Components"
    echo ""
...
menu_install() {
    ...
    echo -e "  ${MENU_NUM_COLOR}5)${C_RESET} Quick Install (NMP Stack)"
    echo ""
...
cmd_purge() {
    ...
    echo -e "${C_RED}Including: Nginx, PHP, MySQL/MariaDB, Redis, and all configurations.${C_RESET}\n"
    ...
    systemctl stop nginx mysql mariadb redis "${php_fpm_units[@]}" 2>/dev/null || true
    systemctl disable nginx mysql mariadb redis "${php_fpm_units[@]}" 2>/dev/null || true
    ...
    apt_remove redis-server redis-tools 2>/dev/null || true
    ...
    rm -rf "${INSTALL_PREFIX}/nginx" "${INSTALL_PREFIX}/php"* "${INSTALL_PREFIX}/redis"
    rm -rf "${PHP_BASE_DIR}" "${ETC_DIR}" "${LOG_DIR}" "${SSL_DIR}"
    ...
    [[ "$keep_data" == "y" ]] && log_info "Data files were preserved"
}
